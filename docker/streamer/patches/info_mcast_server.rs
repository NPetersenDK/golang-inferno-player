...
impl<'s> Multicaster<'s> {
  ...
  // A 6-byte MAC widened to the EUI-64 form Dante uses for clock identities.
  fn eui64_of(mac: &[u8]) -> [u8; 8] {
  ...
  async fn send_clock_stats(&mut self) {
    ...
    if let Some(mc) = master_clock {
      assert_eq!(mc.len(), 8);
      // The file carries the PTPv1 sourceUuid + sourcePortId, but this message
      // wants the MAC alone, zero-padded.
      let gm = &mc[0..6];
      let own = self.self_info.mac_address.octets();

      let mut bytes = ByteBuffer::new();
      bytes.set_endian(bytebuffer::Endian::BigEndian);
      bytes.write_bytes(&[
        0x00, 0x03, 0x00, 0x03, /* 0x01 = PLL not locked */
        0x00, 0x00, 0x00, 0x9f, /* clock stratum: 0x9f follower, 0x29 leader */
      ]);
      bytes.write_i32(freq_offset);
      bytes.write_bytes(&own);
      bytes.write_u16(0);
      bytes.write_bytes(gm);
      bytes.write_u16(0);
      bytes.write_bytes(gm);
      bytes.write_u16(0);

      // Everything below is the port-state table. Dante Controller reads its
      // "Primary v1 Multicast" column from the state word in port record 0, and
      // shows N/A when the table is absent. Captured verbatim from an Audinate
      // follower; only the two clock identities differ per device.
      bytes.write_bytes(&[
        0x00, 0x01, 0x00, 0x34, // sub-block, port table at +0x34
        0x00, 0x09, 0x00, 0x00, 0x02, 0x94, 0x00, 0x00, // summary state 9 = SLAVE
        0x00, 0x03, 0x0d, 0x40, 0x00, 0x00, 0x00, 0x02,
      ]);
      bytes.write_bytes(&[0u8; 16]);
      bytes.write_bytes(&[
        0x00, 0x00, 0x00, 0x01, 0x00, 0x60, 0x0c, 0x00,
        0x00, 0x00, 0x00, 0x0c, 0x00, 0x98, 0x00, 0x20, // identity block at 0x98, len 0x20
        0x00, 0x03, 0x00, 0x00, 0x00, 0x68, 0x10, 0x00, // 3 ports
        // 16 bytes per port; the u16 at +12 is the IEEE 1588 port state.
        0x00, 0x00, 0x00, 0x01, 0x01, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x09, 0x00, 0x07, // v1 primary, SLAVE
        0x00, 0x01, 0x00, 0x02, 0x02, 0x02, 0x02, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x03, 0x00, 0x03, // v2 primary, DISABLED
        0x00, 0x01, 0x00, 0x03, 0x02, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x03, 0x00, 0x07, // DISABLED
        0x00, 0x03, 0x00, 0x07, 0x00, 0xb8, 0x00, 0x04,
      ]);
      // The identity block uses EUI-64, unlike the padded MACs above.
      bytes.write_bytes(&Self::eui64_of(&own));
      bytes.write_bytes(&Self::eui64_of(gm));
      bytes.write_bytes(&Self::eui64_of(gm));
      bytes.write_bytes(&[0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00]);
